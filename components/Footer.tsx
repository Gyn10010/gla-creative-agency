...
const Footer: React.FC<FooterProps> = ({ lang }) => {
  return (
    <footer className="bg-black py-10 border-t border-surface-border">
      <div className="max-w-[1280px] mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-bold text-xl tracking-tighter">GLA</span>
        </div>
        ...
        <p className="text-gray-600 text-sm text-center md:text-right">
          © {new Date().getFullYear()} GLA Design. {lang === 'PT' ? 'Todos os direitos reservados.' : 'All rights reserved.'}
        </p>
      </div>
    </footer>
  );
